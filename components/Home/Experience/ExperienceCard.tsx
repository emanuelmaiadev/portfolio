...
const ExperienceCard = ({service}: Props) => {
  ...
  return (
    <Tilt className='shadow-2xl p-6 rounded-lg bg-[#814ced] h-full'>
      <Image src={service.icon} alt={getTranslatedTitle(service.id)} width={50} height={50}/>
      <h1 className='mt-4 text-lg font-bold text-gray-100'>{getTranslatedTitle(service.id)}</h1>
      <p className='mt-3 text-sm text-white text-opacity-80'>{getTranslatedDescription(service.id)}</p>
    </Tilt>
  );
};
